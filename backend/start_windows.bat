...
IF "%ENV%"=="" SET "ENV=dev"
IF "%GLOBAL_LOG_LEVEL%"=="" SET "GLOBAL_LOG_LEVEL=DEBUG"
...
python -m uvicorn run open_webui.main:app --host "%HOST%" --port "%PORT%" --forwarded-allow-ips '*' --workers %UVICORN_WORKERS% --ws auto
:: For ssl user: python -m uvicorn run open_webui.main:app --host "%HOST%" --port "%PORT%" --forwarded-allow-ips '*' --ssl-keyfile "key.pem" --ssl-certfile "cert.pem" --ws auto
